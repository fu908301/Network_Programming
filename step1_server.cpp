...
void TCP::three_way()
{
  ...
  cout<<"Receive a packet(SYN) from "<<cip<<" : "<<c_port<<endl;
  ...
  cout<<"Send a packet(SYN/ACK) to "<<cip<<" : "<<c_port<<endl;
  recv(clientfd,&_rec,sizeof(_rec),0);
  inet_ntop(AF_INET, &cliaddr.sin_addr,cip,sizeof(cip));
  cout<<"Receive a packet(ACK) from "<<cip<<" : "<<c_port<<endl;
  cout<<"       Receive a packet (seq_num = "<<_rec.seq_num<<", ack_num = "<<_rec.ack_num<<")"<<endl;
...
void TCP::four_way()
{
  ...
  send(clientfd,&_send,sizeof(_send),0);
  cout<<"Send a packet(FIN) to "<<cip<<" : "<<c_port<<endl;
  recv(clientfd,&_rec,sizeof(_rec),0);
  inet_ntop(AF_INET, &cliaddr.sin_addr, cip, sizeof(cip));
  cout<<"Receive a packet(ACK) from "<<cip<<" : "<<c_port<<endl;
  cout<<"       Receive a packet (seq_num = "<<_rec.seq_num<<", ack_num = "<<_rec.ack_num<<")"<<endl;
  recv(clientfd,&_rec,sizeof(_rec),0);
  inet_ntop(AF_INET, &cliaddr.sin_addr, cip, sizeof(cip));
  cout<<"Receive a packet(FIN) from "<<cip<<" : "<<c_port<<endl;
  cout<<"       Receive a packet (seq_num = "<<_rec.seq_num<<", ack_num = "<<_rec.ack_num<<")"<<endl;
  _send.seq_num = _rec.ack_num;
  _send.ack_num = _rec.seq_num + 1;
  send(clientfd,&_send,sizeof(_send),0);
  cout<<"Send a packet (ACK) to "<<cip<<" : "<<cliaddr.sin_port<<endl;
